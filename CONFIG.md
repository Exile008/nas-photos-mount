...
- `忽略 Live Photo，只上传静态图`：同一目录中同时存在同名 `.HEIC` 和 `.MOV` 时，只隐藏配对的 `.MOV`；独立视频不会被过滤。
...
目录卡片中的“暂停挂载”会保存暂停状态并卸载该来源，守护进程不会自动重新挂载。按钮随后变为“恢复挂载”；恢复后继续使用原配置和扫描记录。

## 大目录与 2 TB 内容

挂载不会把整个 NAS 目录下载到手机，文件在 Google Photos 或媒体服务读取时才通过 SMB 按需传输。首次扫描仍需遍历目录元数据；文件非常多时可能耗时较长。

建议先配置忽略规则，将扫描范围缩小，再逐步放开目录。状态面板只显示每个来源的已索引数量、大小、扫描状态和上次扫描时间，也会显示 NAS 总容量和挂载进程状态。Google Photos 的备份判断和云端去重仍由 Google Photos 负责。

首次开启 Live Photo 过滤时，模块需要遍历该来源当前可见的文件名才能识别配对；2 TB 目录可能因此延迟首次挂载。配对清单会缓存在来源状态目录中，后续扫描发现配对变化后更新过滤器并重新挂载。关闭选项即可恢复显示这些 `.MOV`。此功能不会删除已经上传到 Google Photos 的视频。
